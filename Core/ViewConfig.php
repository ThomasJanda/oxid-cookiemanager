...
class ViewConfig extends ViewConfig_parent
{
    ...
    protected $_rs_cookiemanager_cookieId   = null;
    ...
    protected function _rs_cookiemanager_getCookieId()
    {
        if ($this->_rs_cookiemanager_cookieId === null) {
            ...
            if ($sId !== null) {
                ...
                \OxidEsales\Eshop\Core\Registry::getUtilsServer()->setOxCookie($this->_rs_cookiemanager_cookieName,
                    $sId, $iTime);
            }
            $this->_rs_cookiemanager_cookieId = $sId;
        }

        return $this->_rs_cookiemanager_cookieId;
    }
    ...
    protected function _rs_cookiemanager_testShowPopup()
    {
        if ($this->_rs_cookiemanager_testShowPopup === null) {
            if (\OxidEsales\Eshop\Core\Registry::getUtils()->isSearchEngine()) {
                ...
                if ($sActCl === "content") {
                    $aIdents = explode("|",
                        $this->getConfig()->getConfigParam('rs-cookiemanager_hide_cms_ident'));
        ...
        if ($this->_rs_cookiemanager_getActiveCookies === null) {
            ...
            foreach ($aList as $o) {
                ...
                foreach ($aCookie as $oCookie) {
                    ...
                    if (count($aViewClasses) == 0 || in_array($sActCl,
                            $aViewClasses)) {
    ...
    protected function _rs_cookiemanager_getActiveCookiesGrouped()
    {
        if ($this->_rs_cookiemanager_getActiveCookiesGrouped === null) {
            ...
            uksort($this->_rs_cookiemanager_getActiveCookiesGrouped,
                function($a, $b) {
                $a = (int) explode("_", $a)[0];
                $b = (int) explode("_", $b)[0];
        ...
        if ($aCookie = $this->_rs_cookiemanager_getActiveCookiesOfThisView()) {
            foreach ($aCookie as $oCookie) {
                ...
                foreach ($aCookiesTracked as $oCookieTracked) {
                    if ($oCookieTracked->rs_cookie_manager_track__f_rs_cookie_manager->value
                        == $oCookie->rs_cookie_manager_item__f_rs_cookie_manager->value) {
